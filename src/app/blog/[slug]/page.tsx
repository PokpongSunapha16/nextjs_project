...
interface Blog {
  ...
  title: string;
  slug: string;
  picture: string | null; // ✅ รูปภาพของบล็อก
  content: string;
  category: string;
  createdAt: string;
  author: {
    id: number;
    username: string;
    profile_picture: string | null; // ✅ เพิ่มรูปของผู้เขียน
  };
  hearts: number;
  isOwner: boolean;
}
...
export default function BlogDetail() {
    ...
    async function fetchUserStatus() {
      try {
        ...
        if (res.ok && data.isLoggedIn) {
          ...
          setUserId(null); // กำหนดให้เป็น null เมื่อออกจากระบบ
        }
      } catch (err) {
  ...
  useEffect(() => {
    async function fetchBlog() {
      try {
        const res = await fetch(`/api/blog/${slug?.toString()}`);
        const data = await res.json();
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/blog/${slug}/delete`, { method: "DELETE" });
  ...
  return (
    <div className="relative p-6 max-w-4xl mx-auto">
      ...
      <button
            onClick={() => router.push(`/blog/${slug}/report`)}
      ...
      {/* 🏷️ หัวข้อบล็อก */}
      <h1 className="text-4xl font-bold text-center mt-16">{blog.title}</h1>
